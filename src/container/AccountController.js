...
import { TouchableOpacity, ListView, StyleSheet } from 'react-native';
...
import TransactionsController from '../container/TransactionsController';
import { appFonts } from '../util/Theme';
...
function mapStateToProps(state) {
  ...
  return {
    account: account,
    transactions: account.get('transactions') ? account.get('transactions').toArray() : [],
...
class AccountController extends Component {
  ...
  getTokens() {
    ...
    tokens.forEach(token => {
      let view = (
        <Text key={token.getIn(['tokenInfo', 'symbol'])} style={{ color: Colors.BlackAlmost }}>
          {Number(token.get('balance')) / 100 + ' ' + token.getIn(['tokenInfo', 'symbol'])}
        </Text>
      );
      balances.push(view);
    });
  ...
  render() {
    return (
      <ScrollView style={{ flex: 5 }}>
            ...
            >
              <View style={{ flex: 5 }}>
                ...
                <Text style={{ color: Colors.BlackAlmost }}>
                  {this.props.account.getIn(['info', 'ETH', 'balance']) + ' ETH'}
                </Text>
                {this.getTokens()}
              </View>
              <View>
        ...
        <ScrollableTabView
          tabBarUnderlineStyle={{ backgroundColor: Colors.GreenLight, height: 2 }}
          tabBarActiveTextColor={'black'}
          tabBarInactiveTextColor={Colors.GreyMiddle}
          tabBarTextStyle={appFonts.title}
